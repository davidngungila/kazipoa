...
if (-not $flutterInstalled) {
    ...
    try {
        ...
        Write-Host "Downloading from official Flutter repository..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host "Could not add Flutter to PATH automatically" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Could not enable platform support automatically" -ForegroundColor Yellow
}

# Check for Visual Studio
Write-Host "Checking for Visual Studio..." -ForegroundColor Yellow
try {
    $vsInstalled = winget list "Microsoft.VisualStudio.2022.Community" --exact
    if ($vsInstalled -match "No installed package found") {
        Write-Host "Visual Studio not found. Installing Visual Studio Community..." -ForegroundColor Yellow
    ...
    Write-Host "Could not check/install Visual Studio automatically" -ForegroundColor Yellow
}

# Check for Android Studio
Write-Host "Checking for Android Studio..." -ForegroundColor Yellow
try {
    $androidStudio = winget list "Google.AndroidStudio" --exact
    if ($androidStudio -match "No installed package found") {
        Write-Host "Android Studio not found. Installing Android Studio..." -ForegroundColor Yellow
    ...
    Write-Host "Could not check/install Android Studio automatically" -ForegroundColor Yellow
}
